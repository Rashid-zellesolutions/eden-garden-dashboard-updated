...
function Supplies() {
    const [selection, setSelection] = useState("Add Supplies")
    ...
    useEffect(() => {
        const fetchData = () => {
            ...
            fetch(`${Url}Supplies/Get`, requestOptions)
                .then(response => response.json())
                .then(result => {
                    // console.log(result);
                    if (result.status === 200) {

                        setAllSupplies(result.Supplies.reverse())
                        return
                    } else {
                        console.log("error");
                    }
    ...
    const Add = () => {
        ...
        if (!editData) {
            ...
            fetch(`${Url}Supplies/Add`, requestOptions)
                .then(response => response.json())
                .then(result => {
                    setLoading(false)
                    console.log(result);
                    if (result.status == 200) {
                        ...
                        setName("")
                        setAllSupplies([result.Supplies, ...allSupplies])
                        return
                    } else {
            ...
            fetch(`${Url}Supplies/Edit/${editData._id}`, requestOptions)
                .then(response => response.json())
                .then(result => {
                    setLoading(false)
                    console.log(result);
                    if (result.status == 200) {
                        ...
                        setAllSupplies((prevSupplies) =>
                            prevSupplies.map((Supplies) =>
                                Supplies._id === editData._id ? result.updatedSupplies : Supplies
                            )
                        );
                        setSelection("View Supplies")
                        return
                    } else {
                        setErrorPopupMessage(result.message)
                        setErrorPopupOpen(true)
    ...
    const DeleteService = (id) => {
        setLoading(true)
        fetch(
            `${Url}Supplies/Delete/${deleteData._id}`,
            {
                method: "DELETE",
            }
        )
            ...
            .then(result => {
                setLoading(false)
                if (result.status === 200) {
                    setSuccessPopupMessage("Supplies Delete Successfully")
                    setSuccessPopupOpen(true)
                    setAllSupplies((Supplies) => Supplies.filter(Suppliess => Suppliess._id !== deleteData._id));
    ...
    return (
        <div style={{backgroundColor: '#fff', padding: 25}}>
            ...
            <div style={{ display: "flex", alignItems: "center" }}>
                <p onClick={() => setSelection("Add Supplies")} style={{ color: selection === "Add Supplies" ? "#b78953" : "#73787c", fontWeight: "600", cursor: "pointer", fontSize: selection === "Add Supplies" ? "18px" : "14px" }}>Add Supplies</p>
                <p type="primary" onClick={() => setSelection("View Supplies")} style={{ color: selection === "View Supplies" ? "#b78953" : "#73787c", fontWeight: "600", cursor: "pointer", marginLeft: "10px", fontSize: selection === "View Supplies" ? "18px" : "14px" }}>View Supplies</p>
            </div>
            {selection === "Add Supplies" ?
                <>
                    <div style={{ width: "66%", display: "flex", alignItems: "center", gap: 15, marginTop: "20px" }}>
                        <InputField placeholder={"Supplies Name"} label={"Supplies Name"} value={name} onChange={(e) => setName(e.target.value)} width={"48%"} />
                        <div style={{ width: "48%" }}>

                            <Button type="primary" onClick={() => Add()} className="custom-hover-btn btn-text" style={{ height: "35px", marginTop: "10px", background: "#b78953" }}>{editData ? "Save Changes" : "Add Supplies"}</Button>
                        </div>
                    </div>
                </>
                : <div style={{ marginTop: "20px" }}> <Table dataSource={allSupplies} columns={columns} /></div>}
